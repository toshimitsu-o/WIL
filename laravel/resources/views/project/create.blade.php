        ...
        <form action="{{ url('project') }}" method="POST">
            <div class="my-5 space-y-6 rounded-2xl bg-white p-5">
                ...
                <div>
                    ...
                    <textarea class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm mt-1 block w-full" placeholder="Description" name="description">{{ old('description') }}</textarea>
                    <x-input-error class="mt-2" :messages="$errors->get('description')" />
                </div>
                <div>
                    <x-input-label for="capacity" :value="__('Capacity')" />
                    <x-text-input id="capacity" name="capacity" type="number" class="mt-1 block w-full" :value="old('capacity')" />
                    <x-input-error class="mt-2" :messages="$errors->get('capacity')" />
                </div>
                <div>
                    <x-input-label for="email" :value="__('Email Address')" />
                    <x-text-input id="email" name="email" type="text" class="mt-1 block w-full" :value="old('email', Auth::user()->email)" />
                    <x-input-error class="mt-2" :messages="$errors->get('email')" />
                </div>
                <div>
                    <x-input-label for="offer_year" :value="__('Offering Year')" />
                    <x-text-input id="offer_year" name="offer_year" type="number" class="mt-1 block w-full" :value="old('offer_year')" />
                    <x-input-error class="mt-2" :messages="$errors->get('offer_year')" />
                </div>
                <div>
                    <x-input-label for="offer_trimester" :value="__('Offering Trimester')" />
                    <x-text-input id="offer_trimester" name="offer_trimester" type="number" class="mt-1 block w-full" :value="old('offer_trimester')" />
                    <x-input-error class="mt-2" :messages="$errors->get('offer_year')" />
                </div>
